...
![](./pic/MySQL自增主键一定是连续的吗/zoom-4.png)

但如果马上重启 MySQL 实例，重启后这个表的 AUTO_INCREMENT 就会变成 1。﻿ 也就是说，MySQL 重启可能会修改一个表的 AUTO_INCREMENT 的值。

![](./pic/MySQL自增主键一定是连续的吗/zoom-5.png)

![](./pic/MySQL自增主键一定是连续的吗/zoom-6.png)
...
![](./pic/MySQL自增主键一定是连续的吗/zoom-15.png)
...
![](./pic/MySQL自增主键一定是连续的吗/zoom-16.png)

我们创建一个和当前表 `test_pk` 有相同结构定义的表 `test_pk2`：

![](./pic/MySQL自增主键一定是连续的吗/zoom-17.png)

然后使用 `insert...select` 往 `teset_pk2` 表中批量插入数据：

![](./pic/MySQL自增主键一定是连续的吗/zoom-18.png)

可以看到，成功导入了数据。

再来看下 `test_pk2` 的自增值是多少：

![](./pic/MySQL自增主键一定是连续的吗/zoom-19.png)
...
![](./pic/MySQL自增主键一定是连续的吗/zoom-20.png)
